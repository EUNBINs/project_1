...
import com.eunbin.mysolelife.databinding.FragmentBookmarkBinding
...
class BookmarkFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        rv.layoutManager = GridLayoutManager(requireContext(), 2)           // requireContext는 프래그먼트에서 context 넣는 방법
        ...
        val postListener = object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                for (dataModel in dataSnapshot.children) {
                    Log.d(TAG, dataModel.toString())                                // 밑에 categoty1,2의 데이터들 로그가 찍힘
                    val item = dataModel.getValue(ContentModel::class.java)

                    // 북마크한 컨텐츠만 rvlist에 뜨도록 하는 컨텐츠
                    // 3. 전체 컨텐츠 중에서, 사용자가 북마크한 정보만 보여줌
